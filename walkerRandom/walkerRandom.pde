...
void setup() {
  ...
  c = new circle[NCircles];
  for(int i = 0; i < NCircles; i++) {
    c[i] = new circle((int) random(width), (int) random(height));
  } 
...
void mouseClicked(MouseEvent e){
   if(drawnCircles < NCircles){
 c[drawnCircles].pos.x = mouseX;
 c[drawnCircles].pos.y = mouseY;
   }
}
